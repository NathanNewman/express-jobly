...
describe("GET /companies", function () {
  ...
  test("GET /companies Company.search() by name", async () => {
    const resp = await request(app)
      .get("/companies")
      .send({ type: "name", name: "1" });
    expect(resp.statusCode).toBe(200);
    expect(resp.body).toEqual([{ name: "C1" }]);
  });

  test("Fail: GET /companies Company.search() by name", async () => {
    const resp = await request(app)
      .get("/companies")
      .send({ type: "name", name: "A" });
